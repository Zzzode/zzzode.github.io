...
- **Visual changes require a preview review before committing.** Any change to layout, the home page, components, typography, color, images, or article presentation must be served locally (`npm run preview`), screenshotted at desktop (≥1280px) and mobile (390px), and shown to the user for approval. Do not commit or push visual work until the user says it looks right; iterate on the feedback first. Non-visual changes (content text, docs, data fixes) follow the usual check/build gate.
- Home page curation: the lead feature and the first editor's-pick row must be technical content — the latest AI/engineering work, never lifestyle/personal-trip material (which lives inside its article, not in home-page hero real estate). The numbered archive uses an equal-width two-column grid (`grid-cols-2`, not CSS multi-column) so both columns align.
...
- Long-form research write-ups are **distilled articles**: MDX posts (`kind: 'distilled'`) composed with the components in `src/components/article`. When the user hands over a URL / links / pasted material and asks to 炼化 / distill / turn it into an article, invoke the **`distill` skill** (`.agents/skills/distill/SKILL.md`): fetch and cross-check public sources, never fabricate, produce the article per that skill, verify locally, and never push without confirmation.
- Prose quality is gated by the repo-local **`humanize-text` skill** (`.agents/skills/humanize-text/SKILL.md`): distilled articles must pass its humanize step after drafting and before build (no made-up jargon collocations, rule-of-three tics, or scaffold phrases; one natural voice; read-aloud check). It is also invocable on its own for polishing any draft. Facts, numbers, tables and source links are frozen during a humanize pass.
