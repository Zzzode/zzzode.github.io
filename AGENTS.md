...
- Prose quality is gated by the repo-local **`humanize-text` skill** (`.claude/skills/humanize-text/SKILL.md`): distilled articles must pass its humanize step after drafting and before build (no made-up jargon collocations, rule-of-three tics, or scaffold phrases; one natural voice; read-aloud check). It is also invocable on its own for polishing any draft. Facts, numbers, tables and source links are frozen during a humanize pass.
