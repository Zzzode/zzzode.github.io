...
- **Masthead** (`HomeHero.astro`): full-bleed white, centered, compact enough that the lead story begins in the first viewport. Eyebrow → a very large tight H1 set in two tones inline (`ink` phrase then `muted` phrase, on ONE line — do not force a line break with block spans; the title is short) → one gray subhead → blue/ink **text links with `›` chevrons**. Never pill buttons, never a dark hero.
...
- **Lead feature and the highlight rows are reserved for technical content** — the latest AI/engineering writing. Lifestyle or personal-trip pieces stay inside their own article; they never occupy home-page lead or highlight real estate.
- **精选 / Highlights** (`home.picks`): hairline editorial rows (`border-t/border-b border-line` on a `paper` section) with a mono index number, ink title, muted excerpt, right-aligned date — never white rounded cards floating on gray. This is hand-curated and explicitly NOT chronological, so it must never be labeled "最新/latest".
- **最新文章 / Latest** (`home.latest`): a STRICTLY chronological feed on a `white` section — newest first, no manual reordering, lead/highlight items may be excluded as already promoted. Feed items are grouped by calendar day with a full-width date label, and rendered as an **equal-width two-column grid** (`md:grid-cols-2`, never CSS `columns`/multi-column), each item a top-hairline row with a mono index; collapses to one column below `md`. There is **no column gap** (the two halves touch, so paired rules read as one unbroken full-width line; the gutter is made with odd/even cell padding `pr-6`/`pl-6`). The two sections each carry their own heading and rhythm — do not merge curated and chronological rows under a single heading.
...
- **posts**: required `title`, `date`; optional `updated`, `excerpt`, `tags` (default `[]`), `draft` (default `false`); each post gets `/posts/<slug>/` (and `/en/posts/<slug>/`) and enters the matching RSS feed.
  ...
  - **Surface system** (design guide §3.10): distilled articles default to the canvas register — one `<ArticleCanvas>` gray field after the `CoverBanner`, every `<Section canvas>`, blocks as borderless white tiles (`Prose tile`, `Timeline tile`, `CompareTable tile`, `StatGrid tile`, `Summary surface="white"`). Each `SectionHead` gets a sequential `index="01"`… and a short noun-phrase `kicker` (≤6 CJK glyphs) that doubles as the TOC chapter name; never repeat the number inside the kicker text. The floating TOC is liquid glass; its styling and the `--glass-*` tokens are global — do not author per-article TOC styles. Legacy articles may keep `Section alt` surfaces; never mix the two registers in one article.
